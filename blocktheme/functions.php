...
add_filter('render_block_core/post-title', function($block_content, $block){
    $block_content = preg_replace('/<h[1-6](.*?)>/', '<p$1>', $block_content);
    $block_content = preg_replace('/<\/h[1-6]>/', '</p>', $block_content);
...
/* styles: fix layout warnings */
add_action('wp_head', function(){
    echo '<style>
    /* header menu */
    .wp-block-navigation a[href="#"]{pointer-events:none}
    @media(max-width:599px){
        .wp-block-navigation__responsive-container.is-menu-open .wp-block-navigation__responsive-container-content{padding:var(--wp--preset--spacing--large)}
    ...
    .wp-block-button.wc-block-components-product-button{gap:0} a.added_to_cart{display:none}
    /* carousel */
    @media(max-width:599px){
        .wp-block-group-is-layout-grid:has(.wp-block-pullquote){overflow:scroll;scroll-snap-type:x mandatory;grid-template-columns:repeat(6, minmax(min(20rem, 100%), 1fr))}
